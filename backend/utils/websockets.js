...
const { getTokenDetails } = require('./jwtToken');

const { createMeet, addMember } = require('../controllers/meetController');
// const { addMessage } = require('./meets/messages');
// const { createTeamMeet, addTeamMeetMember } = require('./teams/teammeet');
// const {
//   addTeamMeetMessage,
//   addTeamChatMessage,
// } = require('./teams/teammessages');
...
const webSockets = (app) => {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('start meet', async ({ token, meetname }) => {
      //debugging purpose
      console.log('start meet socket called!', token, meetname);
    ...
    socket.on('join room', async ({ roomID, token }) => {
      try {
        ...
        // console.log('roomData', roomData, io.sockets.adapter.rooms);
        // console.log('roomData', roomID, roomData);
        // if (!roomData) {
        //   console.log('invalid roomData');
        //   socket.emit('invalid room');
        //   return;
        // }
        ...
        //   console.log('helo');
        //   // console.log('in loop', io.sockets.connected[key].userName);
    ...
    socket.on('disconnect', () => {
      socket.broadcast
        .to(socket.roomID)
        .emit('disconnected', { id: socket.id, username: socket.userName });
    });
    // whiteboard
    ...
    //   console.log(socket.userName);
    //   socket.broadcast.to(teamid).emit('receive-message', {
    //     sender: socket.userName,
    //     message,
    //     id: socket.id,
    //     date: new Date(),
